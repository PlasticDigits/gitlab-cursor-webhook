...
Find gaps between what the codebase delivers today and what users would reasonably expect — across features, security, UI/UX, testing (unit, integration, e2e), all packages, gas/execution cost, DRY, readability, and language/library best practices.
...
2. Sync the default branch. Explore the full repo — CosmWasm contracts, Terra integrations, indexer, frontend, bot tooling, CI, docs.

3. Run the project test suite and note coverage holes. Start infra per project docs if needed (postgres, LocalTerra, frontend, indexer).
